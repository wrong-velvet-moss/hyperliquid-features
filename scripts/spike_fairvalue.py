...
def main() -> None:
    ...
    md.append(
        "- Signals: `fundingRate` (hourly funding), `premium` (mark-vs-oracle), "
        "`funding_z` (per-coin 1-week z-score of funding)"
    )
    md.append(
        "- Target: forward simple return over 1/4/8/24h. IC = Spearman rank corr.\n"
    )
    ...
    md.append(
        fmt(qt_funding.reset_index()) if not qt_funding.empty else "_insufficient data_"
    )
    md.append(f"\n## Forward {headline_h}h return by `premium` quintile (bps)\n")
    md.append(
        fmt(qt_premium.reset_index()) if not qt_premium.empty else "_insufficient data_"
    )
